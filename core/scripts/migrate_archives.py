...
def parse_args():
    ...
    parsed.which = 'migration'
    if parsed.apply:
        parsed.write_files = True
        parsed.migrate_database = True
    return parsed
...
class Migrator(object):
    ...
    def __init__(self, manager, adapter, archive_def):
        ...
        self.archive_path = os.path.join(self.manager.archive_root,
    ...
    def write_file(self):
        logger.debug('  - Writing snapshot to {}'.format(self.archive_path))
        contents = self.build_archive_data()

        wrote = system.make_file(path=self.archive_path, contents=contents)
        if wrote:
...
class MigrationManager(object):
    def __init__(self, parsed, config):
        self.args = parsed
        self.config = config
        log_path = getattr(self.config, 'log_path', None)
        initialize_logger(parsed.debug, log_path)

        self.archive_root = os.path.normpath(self.config.archive_paths[0])
        system.make_directory(self.archive_root)
    ...
    def perform_migration_with(self, adapter):
        ...
        for idx, migrator in enumerate(archive_defs, start=1):
            ...
            if self.args.write_files:
                migrator.write_file()
                self.archives_written.append(migrator.archive_path)
            else:
    ...
    def migrate(self):
        ...
        if self.archives_written:
            logger.info('The following snapshot files were created:')
            for written in self.archives_written:
...
def main():
    parsed = parse_args()
    cfg = RuntimeConfig.from_args(parsed, allow_archive_configs=True)
    log_path = getattr(cfg, 'log_path', None)
    initialize_logger(parsed.debug, log_path)

    manager = MigrationManager(parsed, cfg)
    try:
        manager.migrate()
    except Exception:
